...
zones = {
    ...
    commands[5]: ['протерти пил', 'помити підлогу', 'викинути сміття', 'відсутність зайвих речей на столах та підлозі',
                  'гардероб: підлогу, відсутність зайвих речей'],
...
async def print_duties(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    start_of_the_message = f'{formatted_date}\n'
    geek = '\n'
    message = f"❗️*Розклад ({formatted_date}):*\n"
    for surnames in command_list:
        message += f"• {surnames}: {', '.join(zones[surnames])}\n"
    command_list.append(command_list.pop(0))
    await context.bot.send_message(chat_id=chat_id, text=start_of_the_message + geek + message + '❗️',
                                   parse_mode='Markdown')
...
❗️Повторне нагадування про прибирання❗️'''
...
if __name__ == '__main__':
    ...
    reminder_handler = CommandHandler('r', reminder)
